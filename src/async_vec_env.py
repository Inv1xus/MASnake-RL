"""
Two vectorized environment backends for Snake PPO training.

AsyncVecEnv runs game logic in subprocess workers and shares observations
through named shared memory blocks to avoid pipe overhead. A global worker
cache means workers stay alive between DEHB trials and are reused rather
than respawned.

TorchSnakeVecEnv runs everything on the GPU with no subprocesses at all.
All game logic is written as batched tensor operations so a full rollout
...
# Fix for Colab and Jupyter: spawn context calls getattr(__main__, '__spec__', None)
# which raises AttributeError in notebook environments where __spec__ does not
# exist as an attribute at all (vs being None). Setting it explicitly
# fixes this.
import types
...
# Reward attributes that can be updated between trials without respawning
# workers
...
def get_cached_vec_env(
    ...
    **kwargs
    ...
    env_backend = str(
        kwargs.get(
            "env_backend",
            "torch" if env_device.type == "cuda" else "async")).lower().strip()
    ...
    if env_backend == "torch":
        ...
        if key in _VECENV_CACHE:
            ...
            vec_env.reset()
            print(
                f"[TorchSnakeVecEnv] Reusing cached env on {env_device}",
                flush=True)
            return vec_env
...
class TorchSnakeVecEnv:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._body_ord = torch.arange(
            self.max_cells, device=self.device).view(
            1, -1)
        self._dir_dx = torch.tensor(
            [0, 0, -1, 1], dtype=torch.long, device=self.device)
        self._dir_dy = torch.tensor(
            [-1, 1, 0, 0], dtype=torch.long, device=self.device)
        self._opp_dir = torch.tensor(
            [1, 0, 3, 2], dtype=torch.long, device=self.device)
        self._speed_growth = torch.tensor(
            SPEED_GROWTH, dtype=torch.float32, device=self.device)
        self._big_dist = self.W + self.H + 1
        self._compile_requested = bool(
            kwargs.get(
                "torch_env_compile",
                True)) and self.device.type == "cuda"
        ...
        self.body_x = torch.zeros(
            state_shape,
            dtype=torch.long,
            device=self.device)
        self.body_y = torch.zeros(
            state_shape,
            dtype=torch.long,
            device=self.device)
        ...
        self.directions = torch.zeros(
            pair_shape, dtype=torch.long, device=self.device)
        self.alive = torch.zeros(
            pair_shape,
            dtype=torch.bool,
            device=self.device)
        self.scores = torch.zeros(
            ...
            dtype=torch.long,
            device=self.device)
        self.food_eaten = torch.zeros(
            pair_shape, dtype=torch.long, device=self.device)
        self.speed_credits = torch.zeros(
            pair_shape, dtype=torch.float32, device=self.device)

        self.food_x = torch.zeros(
            food_shape,
            dtype=torch.long,
            device=self.device)
        self.food_y = torch.zeros(
            food_shape,
            dtype=torch.long,
            device=self.device)
        self.food_active = torch.zeros(
            food_shape, dtype=torch.bool, device=self.device)
        ...
        self.done = torch.zeros(
            ...
            dtype=torch.bool,
            device=self.device)
        self.winner = torch.full((self.num_envs,), -
        ...
        self.torch_rews = torch.zeros(
            pair_shape, dtype=torch.float32, device=self.device)
        self.torch_dones = torch.zeros(
            (self.num_envs, 3), dtype=torch.uint8, device=self.device)
        self.np_grids = None
        self.np_meta = None
        self.np_rews = None
        self.np_dones = None
        ...
        cy = self.H // 2
        self._reset_body_x = torch.zeros(
            (1, 2, self.max_cells), dtype=torch.long, device=self.device)
        self._reset_body_y = torch.zeros(
            (1, 2, self.max_cells), dtype=torch.long, device=self.device)
        self._reset_body_x[0, 0, :3] = torch.tensor(
            [q1, q1 + 1, q1 + 2], dtype=torch.long, device=self.device)
        self._reset_body_x[0, 1, :3] = torch.tensor(
            [q3, q3 - 1, q3 - 2], dtype=torch.long, device=self.device)
        self._reset_body_y[0, :, :3] = cy
        self._reset_lengths = torch.tensor(
            [[3, 3]], dtype=torch.long, device=self.device)
        self._reset_dirs = torch.tensor(
            [[2, 3]], dtype=torch.long, device=self.device)
        self._reset_alive = torch.ones(
            (1, 2), dtype=torch.bool, device=self.device)
        self._reset_pair_long = torch.zeros(
            (1, 2), dtype=torch.long, device=self.device)
        self._reset_pair_float = torch.zeros(
            (1, 2), dtype=torch.float32, device=self.device)
        self._reset_food_active = torch.zeros(
            (1, self.num_food), dtype=torch.bool, device=self.device)
        self._reset_winner = torch.full(
    ...
    def enable_compile(self) -> None:
        ...
        for compile_kwargs in compile_attempts:
            try:
                self._compiled_send_actions = torch.compile(
                    self._send_actions_impl, **compile_kwargs)
        ...
        if last_exc is not None:
            ...
            print(
                f"[TorchSnakeVecEnv] No safe no-cudagraph compile mode available, using eager step path: {last_exc}",
                flush=True)
    ...
    def send_actions(self, actions) -> None:
        ...
        try:
            if self._use_compiled_step and hasattr(
                    torch, "compiler") and hasattr(
            ...
            if self._use_compiled_step:
                ...
                print(
                    f"[TorchSnakeVecEnv] torch.compile fallback to eager step path: {exc}",
                    flush=True)
    ...
    def _send_actions_impl(self, actions_t: torch.Tensor) -> None:
        ...
        # Determine how many blocks each snake moves this tick
        moves = self._plan_tick_moves()
        max_moves = int(moves.max().item())

        # Execute sub-steps until all snakes have taken their full move count
        ...
        self.torch_dones[:, :2] = (terminal_envs.unsqueeze(
            1) | (~self.alive)).to(torch.uint8)
    ...
    def _plan_tick_moves(self) -> torch.Tensor:
        ...
        # Floor the credits to get integer move counts for this tick
    ...
    def _physical_substep(
        ...
        sub_rews = env_active.unsqueeze(
            1).expand(-1, 2).float() * self.survival_reward

        proposed = self.directions
        valid_actions = moving & (actions >= 0) & (actions <= 3)
        proposed = torch.where(valid_actions, actions, proposed)
        reversals = moving & (self.lengths > 1) & (
            proposed == self._opp_dir[self.directions])
        ...
        for aid in (0, 1):
            trim_tail = (moving[:, aid] & (~will_eat[:, aid])
                         & self.alive[:, aid]).long()
        ...
        old_d = (old_x.unsqueeze(-1) - fx).abs() + \
            (old_y.unsqueeze(-1) - fy).abs()
        new_d = (next_x.unsqueeze(-1) - fx).abs() + \
            (next_y.unsqueeze(-1) - fy).abs()
        ...
        sub_rews += self.distance_shaping * \
            (old_d - new_d).float() * shaping_mask.float()

        shifted_x = torch.cat(
            (next_x.unsqueeze(-1), self.body_x[:, :, :-1]), dim=-1)
        shifted_y = torch.cat(
            (next_y.unsqueeze(-1), self.body_y[:, :, :-1]), dim=-1)
        self.body_x = torch.where(
            moved_alive.unsqueeze(-1), shifted_x, self.body_x)
        self.body_y = torch.where(
            moved_alive.unsqueeze(-1), shifted_y, self.body_y)
        ...
        sub_rews[:, 0] += (single_alive & (winners == 0)
                           ).float() * self.win_reward
        sub_rews[:, 1] += (single_alive & (winners == 1)
                           ).float() * self.win_reward
        ...
        self.winner = torch.where(
            step_limit, self._score_winner(), self.winner)
    ...
    def _reset_mask(self, env_mask: torch.Tensor) -> None:
        ...
        self.body_x = torch.where(
            mask3, self._reset_body_x.expand(
                self.num_envs, -1, -1), self.body_x)
        self.body_y = torch.where(
            mask3, self._reset_body_y.expand(
                self.num_envs, -1, -1), self.body_y)
        self.lengths = torch.where(
            mask2, self._reset_lengths.expand(
                self.num_envs, -1), self.lengths)
        self.directions = torch.where(
            mask2, self._reset_dirs.expand(
                self.num_envs, -1), self.directions)
        self.alive = torch.where(
            mask2, self._reset_alive.expand(
                self.num_envs, -1), self.alive)
        self.scores = torch.where(
            mask2, self._reset_pair_long.expand(
                self.num_envs, -1), self.scores)
        self.food_eaten = torch.where(
            mask2, self._reset_pair_long.expand(
                self.num_envs, -1), self.food_eaten)
        self.speed_credits = torch.where(
            mask2, self._reset_pair_float.expand(
                self.num_envs, -1), self.speed_credits)
        self.food_active = torch.where(
            food_mask, self._reset_food_active.expand(
                self.num_envs, -1), self.food_active)
        self.step_count = torch.where(
            env_mask, torch.zeros_like(
    ...
    def _spawn_one_food(self, env_mask: torch.Tensor) -> None:
        ...
        occ = torch.zeros((self.num_envs, self.max_cells),
        ...
        samples = torch.multinomial(
            sample_weights, 1, generator=self._rng).squeeze(1)
        empty_slots = (~self.food_active).long().argmax(dim=1)
        slot_mask = torch.zeros_like(self.food_active)
        slot_mask.scatter_(1,
                           empty_slots.unsqueeze(1),
                           spawn_mask.unsqueeze(1))
        ...
        new_food_x.scatter_(
            ...
            samples.remainder(
                self.W).unsqueeze(1))
        new_food_y.scatter_(1, empty_slots.unsqueeze(1), torch.div(
            samples, self.W, rounding_mode="floor").unsqueeze(1))
    ...
    def _rebuild_observations_impl(self) -> None:
        ...
        self.torch_grids[:, 0, 4].view(
            self.num_envs, -1).scatter_add_(1, food_idx, food_vals)
        self.torch_grids[:, 1, 4].view(
            self.num_envs, -1).scatter_add_(1, food_idx, food_vals)
        ...
        for viewer in (0, 1):
            for src in (0, 1):
                ...
                valid = (
                    self._body_ord < self.lengths[:, src].unsqueeze(-1)) & alive_src.unsqueeze(-1)
                ...
                self.torch_grids[:, viewer, order_ch].view(
                    self.num_envs, -1).scatter_add_(1, body_idx[:, src], order_vals, )
...
def _worker_fn(
    ...
    shms = {
        k: shared_memory.SharedMemory(
            name=v) for k,
        v in shm_names.items()}

    num_local = len(env_kwargs_list)
    sl = slice(env_start, env_start + num_local)

    all_grids = np.ndarray((total_envs, 2, C, H, W),
                           np.float32, buffer=shms["grids"].buf)
    all_meta = np.ndarray((total_envs, 2, META_DIM),
                          np.float32, buffer=shms["meta"].buf)
    ...
    def _write_obs(li: int, raw_obs: dict) -> None:
        """Writes a single env's observation into the shared memory arrays."""
        for aid in (0, 1):
            obs = raw_obs[aid]
    ...
    def _write_step(li: int, rews: dict, dones: dict) -> None:
        """Writes rewards and done flags for a single env into the shared memory arrays."""
...
class AsyncVecEnv:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        n_cpus = mp.cpu_count()
        # Divide by 2 to account for hyperthreading. Cloud VMs report vCPU count
        # (e.g. 32) which is 2x physical cores. Spawning too many Python workers
        # on a small number of physical cores causes massive context-switching
        # overhead.
        ...
        self._shms: dict = {}
        self._shms["grids"], self.np_grids = _alloc(
            (n, 2, C, H, W), np.float32)
    ...
    def _env_location(self, global_i: int):
        ...
        for w, size in enumerate(self._worker_sizes):
            ...
            offset += size
        raise IndexError(
            f"global_i={global_i} out of range (num_envs={self.num_envs})")
