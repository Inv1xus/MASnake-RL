"""
Recurrent PPO with an Inverse Dynamics Model for fully observable Snake.

A GRU processes the observation sequence and an IDM head predicts the action
taken between consecutive hidden states. Gradients from the IDM flow back
through the GRU and CNN so the network learns causally structured features.
"""
...
class EpiplexityTrainer:
    ...
    def __init__(self, config: EpiplexityConfig, device: torch.device):
        ...
        # The critic learns twice as fast because value estimation is harder to converge
        # than policy improvement and it falls behind without the extra LR.
        ...
        # Stores the GRU hidden state at the start of each BPTT window so the update
        # pass re-enters the sequence with the exact state that was used during
        # collection.
    ...
    def train_chunk(self, target_steps: int) -> float:
        ...
        while self.total_steps < target_steps:
            ...
            # l_mask decides which (env, agent) slots belong to the learner this rollout.
            # Opponent slots play from a past snapshot so the learner faces
            # varied opponents.
            l_mask = torch.rand(
            ...
            for t in range(T):
                ...
                # Reset hidden state for any env that just ended so its memory does not
                # contaminate the beginning of the next episode.
                if env_done.any():
                    h_new = h_new.clone()
                    h_new[env_done] = 0.0
                    oh_new = oh_new.clone()
    ...
    def _ppo_update(self, adv, ret, l_mask):
        ...
        for _ in range(self.config.update_epochs):
            ...
            for start in range(0, n_seqs, mb_seqs):
                ...
                if len(mb) == 0:
                    continue
                pi_l, v_l, ent_l = self._bptt_step(
                    gs_seq[mb], ss_seq[mb], acts_seq[mb], lp_old_seq[mb], v_old_seq[mb], adv_seq[mb], ret_seq[mb], h_seq_starts[mb], done_seq[mb])
                pi_sum += pi_l
                v_sum += v_l
                ent_sum += ent_l
                n_mb += 1

        return pi_sum / max(n_mb, 1), v_sum / \
            max(n_mb, 1), ent_sum / max(n_mb, 1)
    ...
    def _bptt_step(
        ...
        with autocast("cuda", enabled=self._use_amp, dtype=self._amp_dtype):
            # The CNN has no sequential dependency so all K*L frames go through in one pass.
            # The GRU loop below must be sequential because state t+1 depends
            # on state t.
            ...
            for t in range(L):
                ...
                logits, value = self._raw_network.heads_from_feat_and_h(
                    feats[:, t], h)
    ...
    def _idm_update(self):
        ...
        # Pairs that straddle an episode boundary are invalid for the IDM because
        # (s_t, a_t, s_t+1) would be mixing observations from two different episodes.
        ...
        self.network.train()
        with autocast("cuda", enabled=self._use_amp, dtype=self._amp_dtype):
            # Process both timesteps in one CNN pass for efficiency
    ...
    def _shm_to_gpu(self):
        ...
        if self._is_torch_env:
            ...
            grids, meta = self._pin_grids.to(
                self.device, non_blocking=True).div_(255.0), self._pin_meta.to(
        ...
        walls = torch.stack([meta[:, :, 3] /
                            ...
                            1)
        scalars = torch.cat(
            [d_oh, meta[:, :, 1:2], meta[:, :, 2:3], walls], dim=-1)
