"""
ppo_snake_core.py (v16 — SPS Recovery & Stability Final)
========================================================
Fixes:
  1. Restored Pinned Memory buffers (Returns SPS to 8,000+).
  2. Restored EXACT original PPOConfig fields for backend compatibility.
  3. Orthogonal initialization & Advantage normalization (Fixes 20M plateau).
  4. Explicit .detach() calls (Fixes Autograd RuntimeError).
"""
...
@dataclass
class PPOConfig:
    # Restored EXACT original field set for strict backend unpacking compatibility
    grid_width:               int   = 24
    grid_height:              int   = 18
    in_channels:              int   = 8
    scalar_dim:               int   = 10
...
class StatefulSnakeTrainer:
    ...
    def __init__(self, config: PPOConfig, device: torch.device):
        ...
        # EXACT original v5 initialization block to ensure backend compatibility
        ...
        # Pinned Memory CPU Buffers (CRITICAL for SPS recovery)
    ...
    def _shm_to_gpu(self):
        # Asynchronous Host->Device transfer using Pinned Memory
        if self._is_torch_env:
            ...
            grids = self._pin_grids.to(self.device, non_blocking=True).div_(255.0)
        ...
        scalars = torch.cat([d_oh, meta[:,:,1:2], meta[:,:,2:3]/50.0, walls], dim=-1)
    ...
    def train_chunk(self, target_steps: int) -> float:
        ...
        while self.total_steps < target_steps:
            ...
            for t in range(T):
                ...
                # Step the environment using the correct backend logic
                if self._is_torch_env:
