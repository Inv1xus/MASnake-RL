"""
Feedforward PPO trainer for fully observable Snake.

Builds the actor-critic network, runs rollouts across vectorized environments,
and updates via clipped PPO with GAE advantage estimation.
"""
...
from async_vec_env import AsyncVecEnv, C, META_DIM, get_cached_vec_env, _REWARD_KEYS
...
def _gae_jit(rb_v, rb_r, rb_d, last_v, gamma: float, gae_lambda: float):
    ...
    # GAE must run backwards because each advantage depends on all future advantages.
    # torch.jit.script compiles this loop to TorchScript so it runs on-device without
    # Python overhead per timestep, which matters when T is in the hundreds.
...
@dataclass
class PPOConfig:
    # Exact original field set for strict backend unpacking compatibility
    grid_width: int = 24
    grid_height: int = 18
    in_channels: int = 8
    scalar_dim: int = 10
...
class StatefulSnakeTrainer:
    ...
    def __init__(self, config: PPOConfig, device: torch.device):
        ...
        self.vec_env = get_cached_vec_env(
            reward_kwargs=reward_kw,
            **cfg_dict,
        )
        ...
        # Pinned memory buffers are critical for SPS recovery on the async
        # backend
    ...
    def _shm_to_gpu(self):
        ...
        if self._is_torch_env:
            ...
            grids = self._pin_grids.to(
                self.device, non_blocking=True).div_(255.0)
        ...
        walls = torch.stack([meta[:, :, 3] /
                            ...
                            1)
        scalars = torch.cat(
            [d_oh, meta[:, :, 1:2], meta[:, :, 2:3] / 50.0, walls], dim=-1)
    ...
    def train_chunk(self, target_steps: int) -> float:
        ...
        while self.total_steps < target_steps:
            ...
            # Buffers are stored as (T, N, A, ...) during collection. We permute to
            # (N, A, T, ...) so l_mask can select learner slots with a single boolean
            # index, then flatten everything into a 1D sample pool for the
            # minibatch loop.
