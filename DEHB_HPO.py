...
def dehb_objective(config, fidelity, **kwargs):
    ...
    checkpoint_dir  = f"dehb_checkpoints_base/trial_{config_id}"
...
if __name__ == "__main__":
    ...
    os.makedirs("dehb_checkpoints_base", exist_ok=True)
    os.makedirs("dehb_log_data_base", exist_ok=True)
    ...
    dehb = DEHB(
        ...
        n_workers=1,
        resume=False,
        output_path="dehb_log_data_base",
    )
    ...
    if dehb.inc_config is not None:
        ...
        with open("best_base_params.json", "w") as f:
