"""
ppo_snake_epiplexity.py  (True Epiplexity — POMDP A/B Test + Aux Tracking)
==========================================================================
Recurrent PPO + IDM + Auxiliary Opponent Tracking.
- Clean Math: Grid division and scalar division bugs removed.
- POMDP: Scalars blinded, 7x7 Fog of War enforced.
- AUX: GRU forced to predict opponent's (X, Y) coordinates to prevent temporal amnesia.
"""
...
class EpiplexityConfig(PPOConfig):
    ...
    aux_loss_coef:     float = 0.5    # Balances the MSE tracking prediction
...
class RecurrentActorCritic(nn.Module):
    def __init__(
        ...
        self.compress = nn.Sequential(layer_init(nn.Linear(cnn_raw + 64, feat_dim)), nn.ReLU())
        ...
        # Auxiliary Head for Opponent (X, Y) Absolute Tracking
    ...
    def heads_from_feat_and_h(self, feat: torch.Tensor, h: torch.Tensor):
        ...
        # Return Logits, Value, and Aux Prediction
...
class EpiplexityTrainer:
    def __init__(self, config: EpiplexityConfig, device: torch.device):
        ...
        # Buffer for the Opponent Answer Key
    ...
    def enable_tracking(self, progress_cb=None):
        self._metrics = {"steps": [], "reward": [], "pi_loss": [], "v_loss": [], "entropy": [], "idm_loss": [], "aux_loss": [], "ep_len": [], "sps": [], "ent_coef": []}
    ...
    def _shm_to_gpu(self):
        ...
        # ── 3. OPPONENT ANSWER KEY EXTRACTION ──
        # Flips the agent dimension [0, 1] -> [1, 0] to match Agent 0 with Agent 1's coords
    ...
    def train_chunk(self, target_steps: int) -> float:
        ...
        while self.total_steps < target_steps:
            ...
            for t in range(T):
                ...
                # Store data, including the opponent coordinate answer key
                self.rb_grids[t], self.rb_scalars[t], self.rb_actions[t], self.rb_log_probs[t], self.rb_values[t], self.rb_opp_coords[t] = gs, ss, acts, lp, v, opp_c
            ...
            if hasattr(self, "_metrics"):
                ...
                m["pi_loss"].append(pi_l); m["v_loss"].append(v_l); m["entropy"].append(ent_l); m["idm_loss"].append(idm_l); m["aux_loss"].append(aux_l)
    ...
    def _ppo_update(self, adv, ret, l_mask):
        ...
        for _ in range(self.config.update_epochs):
            ...
            for start in range(0, n_seqs, mb_seqs):
                ...
                pi_l, v_l, ent_l, aux_l = self._bptt_step(gs_seq[mb], ss_seq[mb], acts_seq[mb], lp_old_seq[mb], v_old_seq[mb], adv_seq[mb], ret_seq[mb], opp_c_seq[mb], h_seq_starts[mb], done_seq[mb])
                pi_sum += pi_l; v_sum += v_l; ent_sum += ent_l; aux_sum += aux_l; n_mb += 1

        return pi_sum / max(n_mb, 1), v_sum / max(n_mb, 1), ent_sum / max(n_mb, 1), aux_sum / max(n_mb, 1)

    def _bptt_step(self, b_gs, b_ss, b_acts, b_lp_old, b_v_old, b_adv, b_ret, b_opp_c, h0, b_done):
        K, L = b_gs.shape[:2]
        ...
        with autocast("cuda", enabled=self._use_amp, dtype=self._amp_dtype):
            ...
            for t in range(L):
                ...
                logits, value, aux_pred = self._raw_network.heads_from_feat_and_h(feats[:, t], h)
                ...
                # Aux MSE Tracking Loss
                aux_loss_t[:, t] = F.mse_loss(aux_pred, b_opp_c[:, t].to(aux_pred.dtype), reduction='none').mean(dim=-1)

                if t < L - 1 and b_done[:, t].any():
                    h = h * (~b_done[:, t]).float().unsqueeze(-1).to(h.dtype)

            pi_loss, v_loss, ent_, aux_loss = pi_loss_t.mean(), v_loss_t.mean(), ent_t.mean(), aux_loss_t.mean()
            loss = pi_loss + self.config.value_loss_coef * v_loss - self._ent_coef * ent_ + self.config.aux_loss_coef * aux_loss
